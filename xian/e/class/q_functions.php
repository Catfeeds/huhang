...
require(ECMS_PATH."e\aliyun\demo\sendSms.php");
//自定义字段返回模板字段处理
...
function AddFeedback($add){
    //$add为页面前端提交过来的信息；
	global $empire,$dbtbpre,$level_r,$public_r,$send;
	...
	if($sql)
	{
        $lastInfo=$empire->query("select * from {$dbtbpre}enewsfeedback where id = $fid");
        $last=mysql_fetch_assoc($lastInfo);
        $mycall = $last['mycall'];
        $name=$last['name'];
        $send = new \Aliyun\DySDKLite\Sms\sendSms();
        //给客户
        $send->sendSms($mycall,'SMS_133977350',$name);
        //给管理员
        $send->sendSmsAdmin('15332335773','SMS_133275287',"新客户姓名".$name."".$mycall,'西安站');
...
function AddError($add){
	global $empire,$class_r,$dbtbpre,$public_r,$send;
	CheckCanPostUrl();//验证来源
	$id=(int)$add['id'];
